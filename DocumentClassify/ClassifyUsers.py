...
def getCategories(cursor):
    cursor.execute("SELECT distinct category FROM 100Famous")
...
def GetClassifyResults(tweets_path):
    ...
    for category in categories:
        # category 是元组格式
        category_dic[category[0]] = 0
    # print category_dic.keys()
    for name in snames:
        # 将字典值都置为0
        map(lambda value:value - value,category_dic.values())
        # 尝试对每一条推文分类后求置信度
        with open(tweets_path + "/" + name,"r") as f:
            lines = f.readlines()
            for line in lines:
                res = TweetsClassify.Classify(line)
                category_dic[res] += 1
            # 将类别求置信
            map(lambda value: value * 1.0 / len(lines),category_dic.values())
            # 返回置信度最大的key
            res = max(category_dic.items(),key = lambda dic : dic[1])
            print "置信度为%f " % category_dic[res]
            # print "%s ==> %s" % (name,res)
            resdic[name] = res
    return resdic
